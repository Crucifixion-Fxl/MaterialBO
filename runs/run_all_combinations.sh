...
OPT_TYPES=("organic" "oxide")
# OBJECTIVE_VERSIONS=("complex" "simple" "standard")
OBJECTIVE_VERSIONS=("paper")
N_ITER=300  # Number of iterations for each combination

TOTAL_COMBINATIONS=$((${#OPT_TYPES[@]} * ${#OBJECTIVE_VERSIONS[@]}))
...
try:
    if '${opt_type}' == 'organic':
        optimizer = run_organic_optimization(objective_version='${objective_version}', n_iter=${N_ITER})
    elif '${opt_type}' == 'oxide':
        optimizer = run_oxide_optimization(objective_version='${objective_version}', n_iter=${N_ITER})
    else:
        print(f"Error: Unknown optimization type: ${opt_type}")
        sys.exit(1)
    
    print(f"SUCCESS: ${opt_type}_${objective_version}")
    print(f"Output directory: {optimizer.output_dir}")
except Exception as e:
    print(f"FAILED: ${opt_type}_${objective_version}")
    print(f"Error: {e}")
    import traceback
    traceback.print_exc()
    sys.exit(1)
...
for opt_type in "${OPT_TYPES[@]}"; do
    for objective_version in "${OBJECTIVE_VERSIONS[@]}"; do
        ...
        COMBINATION_NAME="${opt_type}_${objective_version}"
        ...
        if run_optimization "$opt_type" "$objective_version"; then
            END_TIME=$(date +%s)
